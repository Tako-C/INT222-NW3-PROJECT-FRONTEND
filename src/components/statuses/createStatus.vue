...
async function saveStatusData() {
  let checkStatusName = Store.statuses.filter(
    (status) => status.name === statusData.value.name
  );
    ...
    >
      <div class="rounded-2xl">
        <h1 class="break-words w-[79%]">
          ...
          ><span class="text-[70%] opacity-[60%] font-serif">New Status</span>
        </h1>
      ...
      <div class="mt-3 ml-7">
        ...
        <input
          ...
          maxlength="50"
          class="itbkk-status-name w-[90%] h-8 resize-none italic bg-slate-400 bg-opacity-15 rounded-lg border-2 pl-2"
        />
        ...
        <textarea
          ...
          maxlength="200"
          class="itbkk-status-description w-[90%] h-44 resize-none bg-gray-400 bg-opacity-15 rounded-lg pl-2 overflow-hidden hover:overflow-y-scroll border-2"
        ></textarea>
        <p class="flex justify-end pr-20 text-[10px]">
          {{
