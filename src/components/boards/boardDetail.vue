...
<template>
  ...
  >
    <div class="bg-white rounded-xl shadow-lg w-80 md:w-full max-w-lg p-8">
      <div class="md:hidden">
        <p
          class="text-red-400 flex justify-end cursor-pointer"
          @click="closeModal()"
        >
          x
        </p>
      ...
      <div class="flex space-x-8 text-xs md:text-xl">
        ...
        <div class="flex-none w-auto text-sm space-y-2">
          <div
            class="flex justify-between items-center bg-orange-300 rounded-2xl p-2"
          >
            <p class="font-medium text-gray-700">
              Task<span class="hidden md:inline"> Quantity</span>:
            </p>
            <p class="text-gray-600 font-semibold">
              {{ taskQuantity(boardData.tasks) }}
            </p>
          </div>

          <div
            class="flex justify-between items-center bg-sky-300 rounded-2xl p-2"
          >
            <p class="font-medium text-gray-700">
              Statuses<span class="hidden md:inline"> Quantity</span>:
            </p>
            <p class="text-gray-600 font-semibold">
              {{ taskQuantity(boardData.statuses) }}
            </p>
          </div>
        </div>
      </div>
